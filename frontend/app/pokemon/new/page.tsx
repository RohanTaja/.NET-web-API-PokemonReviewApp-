...
export default function NewPokemonPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setError('Failed to load owners or categories')
      }
    }
    fetchData()
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to create pokemon')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl">
      <h1 className="text-4xl font-bold text-gray-800 mb-8">Create New Pokemon</h1>
